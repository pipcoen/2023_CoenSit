function d
%% This function plots a panel from the manuscript (Figure S2d)
figure;
...
%%
glmBlk.viewGLMFits('simpLogSplitVSplitAAudExtraDom', [],'log', 10, pow2use)
ylabel('p(Rightward)/p(Leftward)')
xlabel('Visual contrast')
set(gca, 'YTickLabel', [0.01, 0.1, 1, 10, 100])
axis square;
title('Extended aud dominance')


subplot(1,2,2);
loglikDat = fit.compareSpecifiedModels({'simpLogSplitVSplitA_Cross5'; 'simpLogSplitVSplitAAudExtraDom_Cross5'});
hold on
xDat = loglikDat.simpLogSplitVSplitA_Cross5;
yDat = loglikDat.simpLogSplitVSplitAAudExtraDom_Cross5;
xlim([0 0.55]);
ylim([0 0.55]);
xlabel('Additive model (bits/trial)')
ylabel('Extended aud dominance model (bits/trial)')
axis square;
