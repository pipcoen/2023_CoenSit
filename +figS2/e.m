function e
%% This function plots a panel from the manuscript (Figure S2e)
figure;
...
%%
glmBlk.viewGLMFits('simpLogSplitVSplitASplitT', [],'log', 10, pow2use)
...
subplot(1,2,2);
loglikDat = fit.compareSpecifiedModels({'simpLogSplitVSplitA_Cross5'; 'simpLogSplitVSplitASplitT_Cross5'});
hold on
xDat = loglikDat.simpLogSplitVSplitA_Cross5;
yDat = loglikDat.simpLogSplitVSplitASplitT_Cross5;
xlim([0 0.55]);
ylim([0 0.55]);
xlabel('Additive model (bits/trial)')
ylabel('Sensory bias model (bits/trial)')
axis square;
