function c
%% This function plots a panel from the manuscript (Figure S2c)
figure;
...
%%
glmBlk.viewGLMFits('simpLogSplitVSplitAAudDom', [],'log', 10, pow2use)
...
subplot(1,2,2);
loglikDat = fit.compareSpecifiedModels({'simpLogSplitVSplitA_Cross5'; 'simpLogSplitVSplitAAudDom_Cross5'});
hold on
xDat = loglikDat.simpLogSplitVSplitA_Cross5;
yDat = loglikDat.simpLogSplitVSplitAAudDom_Cross5;
xlim([0 0.55]);
ylim([0 0.55]);
xlabel('Additive model (bits/trial)')
ylabel('Aud dominance model (bits/trial)')
axis square;
