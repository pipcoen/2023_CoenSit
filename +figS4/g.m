function g
%% This function plots a panel from the manuscript (Figure S4g)
% First do the stats
testSite = 'MOs';
...
figure;
set(gcf, 'position', get(gcf, 'position').*[1 1 1.75, 1])
for z = 1:2
    subplot(1,2,z)
    ...
    title('All params free');
    if z == 2
        set(gca, 'ycolor', 'none')
        title('Bias shift only');
        text(min(xlim)-0.5, min(ylim)+0.5, ['P ~' num2str(pVal)]);
    end
    axis square;
end
end