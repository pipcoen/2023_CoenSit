"""
This scripts generates figure 3g : Choice decoding
...
def main():
    ...
    # load data
    # files_folder_path = '/home/timothysit/Documents/msi-key-data/active-m2-choice-init-decoding/decodeChoiceThreshDir/'
    files_folder_path = '/media/timsit/Partition 1/data/interim/active-m2-choice-init-decoding/decodeChoiceThreshDir'
    ...
    # TOP PANEL: Choice decoding scatter
    plot_sig_star = True
    plot_p_val = True
    verbose = True
    custom_ylim = None
    ...
    # BOTTOM PANEL : STATISITCS COMPARING EACH PAIR / "DICS PLOTS"
    ...
    brain_grouped_accuracies = []

    for brainRegion in np.unique(exp_and_brain_region_grouped_df[brain_region_var_name]):
        brain_region_df = exp_and_brain_region_grouped_df.loc[
            exp_and_brain_region_grouped_df[brain_region_var_name] == brainRegion
            ]

        brain_grouped_accuracies.append(brain_region_df[acc_metric].values)
    ...
    with plt.style.context(splstyle.get_style('nature-reviews')):
        ...
        fig_name = 'active_choiceLeftRight_decoding_all_brain_region_stats_new_order.pdf'
