function p
%% This function plots a panel from the manuscript (Figure S1p)
blks = spatialAnalysis('all', 'behavior', 0, 1, '');
vis2Use = 0.2; % The visual contrast to use for the plot
blks = blks.blks;
...
text(0.7, 95, ['A vs V: P<' num2str(pVal)]);

[~, pVal] = ttest(perf.aud, perf.mul);
pVal = round(pVal, 2, 'significant');
text(1.5, 100, ['A vs A=V: P<' num2str(pVal)]);

[~, pVal] = ttest(perf.vis, perf.mul);
pVal = round(pVal, 2, 'significant');
text(1.5, 95, ['V vs A=V: P<' num2str(pVal)]);
