function f
%% This function plots a panel from the manuscript (Figure 2f)
loadDir = [prc.pathFinder('processeddirectory') 'XSupData\'];
load([loadDir 'fig2fInactResultsForModel'], 'inactResultsForModel')
...
prmOrder = {'visScaleIpsi', 'visScaleConta', 'Bias', 'audScaleIpsi', 'audScaleContra'};
prmTitle = {'Vi', 'Vc', 'b', 'Ai', 'Ac'};
for i = 1:length(prmOrder)
    ...
    if i~=2; scanPlot.legendOff = 1; else; scanPlot.legendOff = 0; end 
    ...
    if i == 1
        ...
        scanPlot.colorBar.Position = scanPlot.colorBar.Position.*[0.7,1,1.25,0.8];
